...
static void foreach_service_friend(gpointer _friend, gpointer _executor)
{
    const RmgFriendResponseEntry *friend = (const RmgFriendResponseEntry *) _friend;

    g_assert(friend);

    rmg_friendtimer_trigger(
        friend->service_name, friend->action, friend->argument, _executor, (guint) friend->delay);
...
static void foreach_service_friend_remove(gpointer data)
{
                                                       ...
                                                       RmgDEvent *dispatcher_event)
{
    ...
    reset_path = rmg_journal_get_public_data_path(
        executor->journal, dispatcher_event->service_name, &error);
    if (error != NULL) {
                                                        ...
                                                        RmgDEvent *dispatcher_event)
{
    ...
    reset_path = rmg_journal_get_private_data_path(
        executor->journal, dispatcher_event->service_name, &error);
    if (error != NULL) {
...
static void do_process_service_restart_event(RmgExecutor *executor, RmgDEvent *dispatcher_event)
{
    ...
    response
        = g_dbus_proxy_call_sync(dispatcher_event->manager_proxy,
                                 "RestartUnit",
...
RmgExecutor *rmg_executor_new(RmgOptions *options, RmgJournal *journal)
{
    ...
    g_source_set_callback(
        RMG_EVENT_SOURCE(executor), NULL, executor, executor_source_destroy_notify);
