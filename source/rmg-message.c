    ...
    case RMG_MESSAGE_REQUEST_FACTORY_RESET:
        msg->hdr.size_of_arg1
            = (uint16_t) (sizeof(gchar)
                          * strnlen(msg->data.service_name, RMG_MESSAGE_MAX_NAME_LEN + 1));
        msg->hdr.size_of_arg2
            = (uint16_t) (sizeof(gchar)
                          * strnlen(msg->data.context_name, RMG_MESSAGE_MAX_NAME_LEN + 1));
        break;

    case RMG_MESSAGE_INFORM_PROCESS_CRASH:
        msg->hdr.size_of_arg1
            = (uint16_t) (sizeof(gchar)
                          * strnlen(msg->data.process_name, RMG_MESSAGE_MAX_NAME_LEN + 1));
        msg->hdr.size_of_arg2
            = (uint16_t) (sizeof(gchar)
                          * strnlen(msg->data.context_name, RMG_MESSAGE_MAX_NAME_LEN + 1));
        break;
