...
static GSourceFuncs checker_source_funcs = {
    checker_source_prepare,
    NULL,
    checker_source_dispatch,
    NULL,
    NULL,
    NULL,
};
...
static gboolean check_services_timer_callback(gpointer user_data)
{
    ...
    rmg_journal_call_foreach_checkstart(
        checker->journal, check_services_callback_for_service, &error);
    return false;
}
...
static void checker_start_check_services_timer(RmgChecker *checker)
{
    ...
    g_timeout_add_seconds_full(
        G_PRIORITY_DEFAULT, (guint) timeout, check_services_timer_callback, checker, NULL);
}
