...
check_services_timer_callback (gpointer user_data)
{
  ...
  rmg_journal_call_foreach_checkstart (checker->journal,
                                       check_services_callback_for_service,
                                       &error);
  return false;
}
...
rmg_checker_new (RmgJournal *journal, RmgOptions *options)
{
  ...
  g_source_set_callback (RMG_EVENT_SOURCE (checker),
                         NULL,
                         checker,
                         checker_source_destroy_notify);
