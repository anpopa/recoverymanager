...
const gchar *active_state_names[]
    = {"unknown", "active", "reloading", "inactive", "failed", "activating", "deactivating", NULL};
...
void proxy_ready_async_cb(GObject *source_object, GAsyncResult *res, gpointer user_data)
{
    ...
    if (error != NULL) {
        ...
        g_signal_connect(
            mentry->proxy, "g-properties-changed", G_CALLBACK(on_properties_changed), mentry);
    }
