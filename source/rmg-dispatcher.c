 ...
 */
static gboolean dispatcher_source_dispatch (GSource *source, GSourceFunc callback,
...
static GSourceFuncs dispatcher_source_funcs = {
  dispatcher_source_prepare, NULL, dispatcher_source_dispatch, NULL, NULL, NULL,
};
...
static gboolean
dispatcher_source_prepare (GSource *source, gint *timeout)
{
...
static gboolean
dispatcher_source_callback (gpointer _dispatcher, gpointer _event)
{
  ...
  switch (event->type)
    ...
    case DEVENT_INFORM_PROCESS_CRASH:
      g_debug ("Dispatch process crash event for '%s' in context '%s'", event->process_name,
               event->context_name);
      do_friend_process_crash_event (dispatcher, event);
      break;

    case DEVENT_INFORM_SERVICE_FAILED:
      g_debug ("Dispatch service failed for '%s' in context '%s'", event->service_name,
               event->context_name);
    ...
    case DEVENT_REMOTE_CONTEXT_RESTART:
      g_info ("Service '%s' from container '%s' request container restart", event->service_name,
              event->context_name);
    ...
    case DEVENT_REMOTE_PLATFORM_RESTART:
      g_info ("Service '%s' from container '%s' request platform restart", event->service_name,
              event->context_name);
    ...
    case DEVENT_REMOTE_FACTORY_RESET:
      g_info ("Service '%s' from container '%s' request factory reset", event->service_name,
              event->context_name);
      ...
      if (*error != NULL)
      ...
      else
        {
          ...
          if (status != RMG_STATUS_OK)
            {
              g_set_error (error, g_quark_from_static_string ("DispatcherNew"), 1,
...
static void
do_relaxtimer_start (RmgJournal *journal, RmgDEvent *event)
{
  ...
  if (error != NULL)
    {
      g_warning ("Fail to trigger relax timer for service %s. Error %s", event->service_name,
                 error->message);
    }
}
...
do_process_service_crash_event (RmgDispatcher *dispatcher, RmgDEvent *event)
{
  ...
  if (error != NULL)
    {
      g_warning ("Fail to read the rvector for service %s. Error %s", event->service_name,
                 error->message);
      return;
    }
  ...
  if (error != NULL)
    {
      g_warning ("Fail to increment the rvector for service %s. Error %s", event->service_name,
                 error->message);
      return;
    }
  ...
  if (error != NULL)
    {
      g_warning ("Fail to read next service action %s. Error %s", event->service_name,
                 error->message);
      return;
    }

  /* check if the action has reset_after flag set */
  action_reset_after = rmg_journal_get_service_action_reset_after (dispatcher->journal,
                                                                   event->service_name, &error);
  if (error != NULL)
    {
      g_warning ("Fail to read action reset after for service %s. Error %s", event->service_name,
                 error->message);
    }
  ...
  if (action_type != ACTION_INVALID)
    {
      g_info ("Action '%s' requiered for service='%s' rvector=%ld",
              rmg_utils_action_name (action_type), event->service_name, rvector);
    }
  ...
  switch (action_type)
      ...
      if (error != NULL)
        {
          g_warning ("Fail to reset rvector after service disable '%s'. Error: %s",
                     event->service_name, error->message);
        }
      break;
      ...
      if (action_reset_after)
        {
          ...
          if (error != NULL)
            {
              g_warning ("Fail to reset rvector for '%s'. Error: %s", event->service_name,
                         error->message);
            }
        }
      else
    ...
    case ACTION_PUBLIC_DATA_RESET:
      rmg_executor_push_event (dispatcher->executor, EXECUTOR_EVENT_SERVICE_RESET_PUBLIC_DATA,
                               event);
      if (action_reset_after)
        {
          rmg_journal_set_rvector (dispatcher->journal, event->service_name, 0, &error);
    ...
    case ACTION_PRIVATE_DATA_RESET:
      rmg_executor_push_event (dispatcher->executor, EXECUTOR_EVENT_SERVICE_RESET_PRIVATE_DATA,
                               event);
      if (action_reset_after)
        {
          rmg_journal_set_rvector (dispatcher->journal, event->service_name, 0, &error);
...
static void
foreach_client_notify_process_crash (gpointer _client, gpointer _event)
{
  ...
  if (rmg_client_send (client, msg) != RMG_STATUS_OK)
    ...
    {
      g_debug ("Replica instance '%s' informed about process '%s'", client->context_name,
               event->process_name);
    }
}

static void
do_friend_process_crash_event (RmgDispatcher *dispatcher, RmgDEvent *event)
{
  g_assert (dispatcher);
  g_assert (event);
...
static void
foreach_client_notify_service_failed (gpointer _client, gpointer _event)
{
  RmgClient *client = (RmgClient *)_client;
  RmgDEvent *event = (RmgDEvent *)_event;
      ...
      if (rmg_client_send (client, msg) != RMG_STATUS_OK)
      ...
      else
        {
          g_debug ("Replica instance '%s' informed about service failure for '%s'",
                   client->context_name, event->service_name);
        }
    }
}

static void
do_friend_service_failed_event (RmgDispatcher *dispatcher, RmgDEvent *event)
{
  g_assert (dispatcher);
  g_assert (event);
  ...
  if (g_run_mode == RUN_MODE_PRIMARY)
  ...
  else
    {
      ...
      if (rmg_manager_send (dispatcher->manager, msg) != RMG_STATUS_OK)
        ...
        g_debug ("Primary instance informed about service failure for '%s'", event->service_name);
    }
...
rmg_dispatcher_new (RmgOptions *options, RmgJournal *journal, RmgExecutor *executor, GError **error)
{
  ...
  if (run_mode_specific_init (dispatcher, error) == RMG_STATUS_OK)
    {
      g_source_set_callback (RMG_EVENT_SOURCE (dispatcher), NULL, dispatcher,
