 ...
 */
static gboolean dispatcher_source_dispatch (GSource *source,
 ...
 */
static gboolean dispatcher_source_callback (gpointer _dispatcher,
                                            gpointer _event);
...
do_process_service_crash_event (RmgDispatcher *dispatcher, RmgDEvent *event)
{
  ...
  switch (action_type)
    ...
    case ACTION_PUBLIC_DATA_RESET:
      rmg_executor_push_event (dispatcher->executor,
                               EXECUTOR_EVENT_SERVICE_RESET_PUBLIC_DATA,
                               event);
      if (action_reset_after)
        {
          rmg_journal_set_rvector (dispatcher->journal,
                                   event->service_name,
                                   0,
                                   &error);
          if (error != NULL)
...
foreach_client_notify_process_crash (gpointer _client, gpointer _event)
{
  ...
  if (rmg_client_send (client, msg) != RMG_STATUS_OK)
    {
      g_warning ("Fail to send crash information to replica instance %s",
                 client->context_name);
    }
  else
    {
...
static void
do_friend_process_crash_event (RmgDispatcher *dispatcher,
                               RmgDEvent *event)
{
  g_assert (dispatcher);
  g_assert (event);
  ...
  if (g_run_mode == RUN_MODE_PRIMARY)
    {
      g_list_foreach (dispatcher->server->clients,
                      foreach_client_notify_process_crash,
                      event);
    }
...
do_friend_service_failed_event (RmgDispatcher *dispatcher, RmgDEvent *event)
{
  ...
  if (g_run_mode == RUN_MODE_PRIMARY)
    {
      g_list_foreach (dispatcher->server->clients,
                      foreach_client_notify_service_failed,
                      event);
    }
  else
    {
