...
gint find_service_by_name(gconstpointer _entry, gconstpointer _service_name)
{
                              ...
                              gpointer user_data)
{
    ...
    if (g_strcmp0(signal_name, "UnitNew") == 0) {
        ...
        if ((service_name != NULL) && (object_path != NULL)) {
            add_service(
                monitor, service_name, object_path, SERVICE_STATE_INACTIVE, SERVICE_SUBSTATE_DEAD);
        } else
                        ...
                        ServiceActiveSubstate active_substate)
{
    ...
    if (check_existent == NULL) {
        ...
        rmg_mentry_build_proxy_async(
            entry, monitor->dispatcher, mentry_init_status, (gpointer) monitor);
    }
}
...
static void monitor_read_services(RmgMonitor *monitor)
{
    ...
    service_list = g_dbus_proxy_call_sync(
        monitor->proxy, "ListUnits", NULL, G_DBUS_CALL_FLAGS_NONE, -1, NULL, &error);
    if (error != NULL)
...
static void monitor_add_relax_timer(gpointer _journal, gpointer _data)
{
    ...
    if (error != 0) {
        g_warning(
            "Fail to trigger relaxtimer for service %s. Error %s", service_name, error->message);
    }
}
...
static void monitor_start_relax_timers(RmgMonitor *monitor)
{
    ...
    rmg_journal_call_foreach_relaxing(
        monitor->dispatcher->journal, monitor_add_relax_timer, &error);

    if (error != 0)
        g_warning("Fail to start relax timers. Error %s", error->message);
}

static void remove_service_entry(gpointer _entry)
{
