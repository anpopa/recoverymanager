...
static GSourceFuncs crashmonitor_source_funcs = {
    crashmonitor_source_prepare,
    NULL,
    crashmonitor_source_dispatch,
    NULL,
    NULL,
    NULL,
};
...
crashmonitor_source_dispatch(GSource *source, GSourceFunc callback, gpointer _crashmonitor)
{
    ...
    return crashmonitor->callback(crashmonitor, event) == TRUE ? G_SOURCE_CONTINUE
                                                               : G_SOURCE_REMOVE;
}
                              ...
                              gpointer user_data)
{
    ...
    if (g_strcmp0(signal_name, "NewCrash") == 0) {
        ...
        if ((proc_name != NULL) && (proc_context != NULL) && (proc_crashid != NULL)) {
            ...
            g_debug("Dispatch process crash information for %s in context %s, crashid %s",
                    proc_name,
                    proc_context,
                    proc_crashid);
...
static void crashmonitor_build_proxy(RmgCrashMonitor *crashmonitor)
{
    ...
    crashmonitor->proxy = g_dbus_proxy_new_for_bus_sync(G_BUS_TYPE_SYSTEM,
                                                        G_DBUS_PROXY_FLAGS_NONE,
                                                        NULL, /* GDBusInterfaceInfo */
                                                        cdm_dbus_name,
                                                        cdm_dbus_object_path,
                                                        cdm_dbus_interface_crashes,
                                                        NULL, /* GCancellable */
                                                        &error);
    ...
    if (error != NULL)
        ...
        g_signal_connect(
            crashmonitor->proxy, "g-signal", G_CALLBACK(on_manager_signal), crashmonitor);
...
RmgCrashMonitor *rmg_crashmonitor_new(RmgDispatcher *dispatcher)
{
    ...
    g_source_set_callback(
        RMG_EVENT_SOURCE(crashmonitor), NULL, crashmonitor, crashmonitor_source_destroy_notify);
...
void rmg_crashmonitor_register_proxy_available_callback(RmgCrashMonitor *crashmonitor,
