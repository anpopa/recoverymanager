...
static gint64 get_long_option (RmgOptions *opts, const gchar *section_name,
...
rmg_options_string_for (RmgOptions *opts, RmgOptionsKey key)
{
  switch (key)
    ...
    case KEY_PRIVATE_DATA_RESET_CMD:
      if (opts->has_conf)
        {
          gchar *tmp = g_key_file_get_string (opts->conf, "recoverymanager",
                                              "PrivateDataResetCommand", NULL);
...
static gint64
get_long_option (RmgOptions *opts, const gchar *section_name, const gchar *property_name,
                 GError **error)
{
  g_assert (opts);
  ...
  g_set_error_literal (error, g_quark_from_string ("rmg-options"), 0,
