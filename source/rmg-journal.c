    ...
    case QUERY_GET_CHECK_START:
        for (gint i = 0; i < argc; i++) {
            if (g_strcmp0(colname[i], "CHKSTART") == 0)
                *((gboolean *) (querydata->response))
                    = (gboolean) g_ascii_strtoll(argv[i], NULL, 10);
        }
        break;
...
RmgJournal *rmg_journal_new(RmgOptions *options, GError **error)
{
    ...
    if (sqlite3_open(dbfile, &journal->database)) {
        ...
        if (sqlite3_exec(journal->database, services_sql, sqlite_callback, &data, &query_error)
            ...
            g_set_error(
                error, g_quark_from_static_string("JournalNew"), 1, "Create services table fail");
        } else {
                                 ...
                                 GError **error)
{
    ...
    if (g_strcmp0(element_name, "action") == 0) {
        ...
        if (retry < 1 || action_type == ACTION_INVALID)
            ...
            rmg_jentry_add_action(
                entry, action_type, rmg_jentry_get_rvector(entry), g, reset_after);
...
static void add_action_for_service(gpointer _action, gpointer _helper)
{
    ...
    g_info(
        "Adding action='%s' for service='%s'", g_action_name[action->type], helper->service->name);
...
static void add_friend_for_service(gpointer _friend, gpointer _helper)
{
...
RmgStatus rmg_journal_reload_units(RmgJournal *journal, GError **error)
{
    ...
    while ((nfile = g_dir_read_name(gdir)) != NULL) {
        ...
        if (!g_markup_parse_context_parse(
                parser_context, fdata, (gssize) strlen(fdata), &element_error)) {
            ...
            if (rmg_journal_remove_service(journal, jentry->name, &element_error)
                != RMG_STATUS_OK) {
...
rmg_journal_get_private_data_path(RmgJournal *journal, const gchar *service_name, GError **error)
{
    ...
    sql = g_strdup_printf(
        "SELECT PRIVDATA FROM %s WHERE NAME IS '%s'", rmg_table_services, service_name);

    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalGetPrivateData"), 1, "SQL query error");
...
rmg_journal_get_public_data_path(RmgJournal *journal, const gchar *service_name, GError **error)
{
    ...
    sql = g_strdup_printf(
        "SELECT PUBLDATA FROM %s WHERE NAME IS '%s'", rmg_table_services, service_name);

    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalGetPublicData"), 1, "SQL query error");
...
gboolean rmg_journal_get_checkstart(RmgJournal *journal, const gchar *service_name, GError **error)
{
    ...
    sql = g_strdup_printf(
        "SELECT CHKSTART FROM %s WHERE NAME IS '%s'", rmg_table_services, service_name);

    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalGetCheckStart"), 1, "SQL query error");
                                       ...
                                       GError **error)
{
    ...
    sql = g_strdup_printf(
        "SELECT TIMEOUT FROM %s WHERE NAME IS '%s'", rmg_table_services, service_name);

    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalGetRelaxingTimeout"), 1, "SQL query error");
                                         ...
                                         GError **error)
{
    ...
    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalCallCheckStart"), 1, "SQL query error");
...
glong rmg_journal_get_rvector(RmgJournal *journal, const gchar *service_name, GError **error)
{
    ...
    sql = g_strdup_printf(
        "SELECT RVECTOR FROM %s WHERE NAME IS '%s'", rmg_table_services, service_name);
                                  ...
                                  GError **error)
{
    ...
    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalSetRelaxingState"), 1, "SQL query error");
...
rmg_journal_get_service_action(RmgJournal *journal, const gchar *service_name, GError **error)
{
    ...
    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalGetServiceAction"), 1, "SQL query error");
                                           ...
                                           GError **error)
{
    ...
    if (sqlite3_exec(journal->database, sql, sqlite_callback, &data, &query_error) != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalGetServicesForFriend"), 1, "SQL query error");
...
RmgStatus rmg_journal_remove_service(RmgJournal *journal, const gchar *service_name, GError **error)
{
    ...
    if (sqlite3_exec(journal->database, service_sql, sqlite_callback, &data, &query_error)
        != SQLITE_OK) {
        g_set_error(
            error, g_quark_from_static_string("JournalRemoveService"), 1, "SQL query error");
        ...
        actions_sql = g_strdup_printf(
            "DELETE FROM %s WHERE SERVICE IS '%s'", rmg_table_actions, service_name);
        ...
        friends_sql = g_strdup_printf(
            "DELETE FROM %s WHERE SERVICE IS '%s'", rmg_table_friends, service_name);
...
gulong rmg_journal_get_hash(RmgJournal *journal, const gchar *service_name, GError **error)
{
    ...
    sql = g_strdup_printf(
        "SELECT HASH FROM %s WHERE NAME IS '%s'", rmg_table_services, service_name);
