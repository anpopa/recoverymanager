    ...
    case RMG_MESSAGE_INFORM_PRIMARY_SERVICE_FAILED:
      {
        ...
        g_info ("Dispatch replica instance service failed '%s' for '%s'", event->service_name,
                event->context_name);
        rmg_dispatcher_push_service_event (dispatcher, event);
      }
      break;
...
rmg_client_new (gint clientfd, gpointer dispatcher)
{
  ...
  g_source_set_callback (RMG_EVENT_SOURCE (client), G_SOURCE_FUNC (client_source_callback), client,
                         client_source_destroy_notify);
  g_source_attach (RMG_EVENT_SOURCE (client), NULL);

  client->tag
      = g_source_add_unix_fd (RMG_EVENT_SOURCE (client), client->sockfd, G_IO_IN | G_IO_PRI);
