...
static gboolean relaxtimer_callback(gpointer user_data)
{
    ...
    current_rvector
        = rmg_journal_get_rvector(relaxtimer->journal, relaxtimer->service_name, &error);
    if (error != NULL) {
...
void relaxtimer_destroy_notify(gpointer data)
{
