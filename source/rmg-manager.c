...
static GSourceFuncs manager_source_funcs = {
  manager_source_prepare, NULL, manager_source_dispatch, NULL, NULL, NULL,
};

static gboolean
manager_source_prepare (GSource *source, gint *timeout)
{
  RMG_UNUSED (source);
  *timeout = -1;
  return FALSE;
...
static gboolean
manager_source_check (GSource *source)
{
  RMG_UNUSED (source);
  return TRUE;
}
...
static gboolean
manager_source_dispatch (GSource *source, GSourceFunc callback, gpointer user_data)
{
  RMG_UNUSED (source);
...
static gboolean
manager_source_callback (gpointer data)
{
...
static void
process_message (RmgManager *manager, RmgMessage *msg)
{
    ...
    case RMG_MESSAGE_INFORM_PROCESS_CRASH:
      {
        ...
        g_info ("Primary instance informed about process '%s' crash in context '%s'",
                event->process_name, event->context_name);
    ...
    case RMG_MESSAGE_INFORM_CLIENT_SERVICE_FAILED:
      {
        ...
        g_info ("Primary instance informed about service '%s' fail in context '%s'",
                event->service_name, event->context_name);
        rmg_dispatcher_push_service_event (dispatcher, event);
      }
      break;
...
static void
manager_source_destroy_notify (gpointer data)
{
  RmgManager *manager = (RmgManager *)data;
...
static void
send_descriptor (RmgManager *manager)
{
  g_autoptr (RmgMessage) msg = NULL;
...
RmgManager *
rmg_manager_new (RmgOptions *opts, gpointer dispatcher)
{
  ...
  g_source_set_callback (RMG_EVENT_SOURCE (manager), G_SOURCE_FUNC (manager_source_callback),
                         manager, manager_source_destroy_notify);
...
RmgManager *
rmg_manager_ref (RmgManager *manager)
{
  g_assert (manager);
...
void
rmg_manager_unref (RmgManager *manager)
{
  g_assert (manager);
...
RmgStatus
rmg_manager_connect (RmgManager *manager)
{
...
RmgStatus
rmg_manager_disconnect (RmgManager *manager)
{
  if (!manager->connected)
    return RMG_STATUS_ERROR;
...
gboolean
rmg_manager_connected (RmgManager *manager)
{
  g_assert (manager);
...
RmgStatus
rmg_manager_send (RmgManager *manager, RmgMessage *m)
{
  fd_set wfd;
  struct timeval tv;
