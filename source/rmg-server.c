...
RmgServer *
rmg_server_new (RmgOptions *options, gpointer dispatcher, GError **error)
{
  ...
  g_source_set_callback (RMG_EVENT_SOURCE (server), G_SOURCE_FUNC (server_source_callback), server,
...
static void
clients_list_destroy_notify (gpointer _client)
{
...
void
rmg_server_add_client (RmgServer *server, gpointer client)
{
  g_assert (server);
  g_assert (client);
...
void
rmg_server_rem_client (RmgServer *server, gpointer client)
{
  g_assert (server);
  g_assert (client);
...
static gint
foreach_client_compare (gconstpointer _client, gconstpointer _context_name)
{
...
gpointer
rmg_server_get_client (RmgServer *server, const gchar *context_name)
{
  GList *el = NULL;
